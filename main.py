...
def generate_matrix():
    ...
    if resp.status_code != 200:
        raise HTTPException(status_code=resp.status_code, detail="Ошибка запроса в Pyrus")

    try:
        data = resp.json()
    except Exception:
        raise HTTPException(status_code=500, detail="Некорректный JSON в ответе Pyrus")
