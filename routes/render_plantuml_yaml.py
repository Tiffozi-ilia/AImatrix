...
# ---------- НОВЫЙ эндпойнт: сырой YAML/текст без экранирования ----------

@router.post("/render_plantuml_yaml")
async def render_plantuml_yaml(
    fmt: Literal["png","svg","txt"],
    code: str = Body(
        ...,
        media_type="text/plain",
        description="Сырой PlantUML-текст (можно присылать как text/plain или application/x-yaml). Никакого экранирования не требуется.",
    ),
):
    """
    POST /render_plantuml_yaml?fmt=png|svg|txt
    Body (raw): <сырое содержимое диаграммы PlantUML, без экранирования>

    Этот маршрут НИЧЕГО не разэкранирует и не трогает кавычки.
    Он только (опционально) оборачивает в @startuml/@enduml, если пользователь не добавил их сам.
    """
    if not code or not code.strip():
        raise HTTPException(400, detail="Empty PlantUML code")

    raw = code  # без любых преобразований/разэкранирований
    log.info("YAML RAW len=%d head=%r", len(raw), raw[:120].replace("\n", "\\n"))
